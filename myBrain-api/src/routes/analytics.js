...
/**
 * Claude usage service specifically handles tracking Claude Code CLI usage.
 * This is separate from regular analytics because it processes data from
 * the ccusage CLI tool (a special format with daily usage summaries).
 */
...
// =============================================================================
// CLAUDE CODE USAGE TRACKING - Track Claude API Usage
// =============================================================================
// These endpoints handle tracking Claude Code CLI usage from the ccusage tool.
// This allows users to see how much they're spending on Claude API calls.

/**
 * POST /analytics/claude-usage
 * Sync Claude Code usage data from ccusage CLI tool
 *
 * The ccusage CLI tool (installed alongside Claude Code) tracks:
 * - When you use Claude Code commands
 * - How many tokens were used per day
 * - Daily and total costs
 * - Model usage breakdown
 *
 * WHAT IS ccusage?
 * ---------------
 * ccusage is a command-line tool that reads your Claude API usage.
 * It can export usage data as JSON (which this endpoint accepts).
 *
 * HOW IT WORKS:
 * -----------
 * 1. User runs: ccusage export-json
 * 2. Gets JSON with daily usage data
 * 3. Sends to this endpoint: POST /analytics/claude-usage
 * 4. We store it in database (daily records)
 * 5. User can view costs/usage anytime
 *
 ...
 *       totalCost: 0.45,
 *       tokenCount: 15000,
 *       models: { "claude-haiku": 10000, "claude-opus": 5000 }
 *     },
 *     ...
 *   ]
 * }
 ...
 * WHY TRACKING MATTERS:
 * --------------------
 * - Users want to know their API costs
 * - Help estimate monthly usage patterns
 * - Identify if usage is increasing/decreasing
 * - Plan for budget limits
 *
 * @body {object} usageData - ccusage JSON export data (required)
 * @returns {Object} Processing result with days processed, total cost
 */
router.post('/claude-usage', requireAuth, async (req, res, next) => {
  try {
    ...
    // =============================================================================
    // STEP 1: Validate Usage Data Format
    // =============================================================================
    // Check that usageData exists and has the "daily" array
    // WHY: Without proper format, we can't parse it
    if (!usageData?.daily) {
      return res.status(400).json({
        success: false,
        error: {
    ...
    // =============================================================================
    // STEP 2: Record Sync Event and Process Daily Data
    // =============================================================================
    // NEW: recordSyncEvent creates two types of records:
    // 1. ClaudeUsageSync - Complete sync event with comparison
    // 2. ClaudeUsage - Daily records (existing functionality)
    ...
    // =============================================================================
    // STEP 3: Log the Sync Event for Audit Trail
    // =============================================================================
    // Attach user ID and sync ID to request for logging
    ...
    res.status(201).json({
      success: true,
      data: {
        ...result.dailyProcessing,  // daysProcessed, totalCost, dateRange
        sync: {
          id: result.sync._id,
...
/**
 * GET /analytics/claude-usage
 * Retrieve Claude Code usage stats for the current user
 *
 * This endpoint lets users view their Claude API usage history.
 * They can query:
 * - Recent usage (last 30 days by default)
 * - Custom date ranges
 * - Daily breakdown
 * - Cost summaries
 *
 * EXAMPLE USES:
 * -----------
 * - User checking "how much did I spend this month?"
 * - Verifying usage accuracy (did the costs match my bill?)
 * - Analyzing usage trends (am I using Claude Code more?)
 * - Planning budget allocation
 *
 * @query {number} days - Last N days (default 30)
 * @query {string} startDate - Custom start date
 * @query {string} endDate - Custom end date
 * @returns {Object} Usage stats with daily breakdown and totals
 */
router.get('/claude-usage', requireAuth, async (req, res, next) => {
  try {
    // Extract date range parameters
    const { startDate, endDate, days = 30 } = req.query;

    // =============================================================================
    // STEP 1: Fetch Usage Stats for Date Range
    // =============================================================================
    // Support two query modes: custom dates OR last N days
    let stats;
    if (startDate && endDate) {
      // Custom date range provided
      stats = await claudeUsageService.getUsageStats(
        req.user._id,
        new Date(startDate),
        new Date(endDate)
      );
    } else {
      // Default: last N days (default 30 days)
      stats = await claudeUsageService.getRecentUsage(
        req.user._id,
        parseInt(days, 10)
      );
    }
    ...
    // =============================================================================
    // Attach user ID for audit trail
    attachEntityId(req, 'userId', req.user._id);
    // Set event name for logging
    req.eventName = 'claude_usage.view';

    // =============================================================================
    // STEP 3: Return Usage Stats
    // =============================================================================
    res.json({
      success: true,
      data: stats  // Daily usage, costs, summaries
    });
  } catch (err) {
    next(err);
  }
...
/**
 * GET /analytics/claude-usage/last-sync
 * Get timestamp of most recent usage data sync
 *
 * This endpoint returns when the user last synced their Claude Code usage data.
 * Used by the frontend to:
 * - Display "Last synced X minutes ago"
 * - Show warnings if data is stale (>1 hour old)
 * - Determine if the "Refresh" button should prompt user to run /claude-usage
 *
 * WHY THIS MATTERS:
 * ----------------
 * - Users want to know if their stats are current
 * - Stale data (>1 hour) should prompt them to re-sync
 * - Helps users understand when they last updated their usage tracking
 *
 * RETURN FORMAT:
 * -------------
 * {
 *   success: true,
 *   data: {
 *     lastSyncAt: Date("2026-01-21T10:30:00.000Z") or null,
 *     recordCount: 30
 *   }
 * }
 *
 * @returns {Object} Last sync timestamp and total record count
 */
router.get('/claude-usage/last-sync', requireAuth, async (req, res, next) => {
  try {
    // =============================================================================
    // STEP 1: Get Latest Sync Event (NEW)
    // =============================================================================
    // Use the new sync tracking system to get the actual sync timestamp
    // This updates every time /claude-usage is run, even if daily data unchanged
    const latestSyncEvent = await claudeUsageService.getLatestSyncInfo(req.user._id);

    // =============================================================================
    // STEP 2: Get Daily Record Count for Backward Compatibility
    // =============================================================================
    // Keep returning recordCount for existing UI components
    const dailyStats = await claudeUsageService.getLatestSync(req.user._id);

    // =============================================================================
    // STEP 3: Log the View Event
    // =============================================================================
    // Attach user ID for audit trail
    attachEntityId(req, 'userId', req.user._id);
    if (latestSyncEvent) {
      attachEntityId(req, 'syncId', latestSyncEvent._id);
    }
    // Set event name for Wide Events logging
    req.eventName = 'claude_usage.last_sync';

    // =============================================================================
    // STEP 4: Return Sync Info (Using New Sync Timestamp with Fallback)
    // =============================================================================
    // Primary: Use ClaudeUsageSync.syncedAt (new model - actual sync timestamp)
    // Fallback: Use ClaudeUsage.createdAt (old model - when daily record was created)
    // This ensures backward compatibility for users who have daily records
    // but haven't run a sync with the new ClaudeUsageSync model yet
    res.json({
      success: true,
      data: {
        lastSyncAt: latestSyncEvent?.syncedAt || dailyStats?.date || null,
        recordCount: dailyStats?.count || 0
      }
    });
  } catch (err) {
    next(err);
...
/**
 * GET /analytics/claude-usage/syncs
 * Get sync history for the current user
 *
 * This endpoint returns a history of Claude usage sync events.
 * Users can see:
 * - When they ran /claude-usage skill
 * - How much each sync included
 * - Comparison between syncs
 * - Trends over time
 *
 * USE CASES:
 * ---------
 * - View last N syncs to see usage growth
 * - Compare this week vs last week
 * - Build graphs showing usage trends
 * - Identify when usage spiked
 *
 * @query {number} limit - Number of syncs to return (default 10, max 50)
 * @returns {Object} Array of sync events with summaries and comparisons
 */
router.get('/claude-usage/syncs', requireAuth, async (req, res, next) => {
  try {
    // Extract limit parameter
    const limit = Math.min(parseInt(req.query.limit, 10) || 10, 50);

    // =============================================================================
    // STEP 1: Fetch Sync History
    // =============================================================================
    // Get last N sync events for this user
    const syncs = await claudeUsageService.getSyncHistory(req.user._id, limit);
    ...
    attachEntityId(req, 'userId', req.user._id);
    // Set event name for Wide Events logging
    req.eventName = 'claude_usage.syncs.view';

    // =============================================================================
    // STEP 3: Return Sync History
    // =============================================================================
    res.json({
      success: true,
      data: {
...
/**
 * GET /analytics/claude-usage/syncs/latest
 * Get the most recent sync event with comparison
 *
 * This endpoint returns the latest sync event, which includes:
 * - Complete sync summary (days, cost, tokens)
 * - Comparison to previous sync (deltas)
 * - Raw data from ccusage
 *
 * USE CASES:
 * ---------
 * - Display "since last sync" stats
 * - Show latest usage summary
 * - Check if sync data is available
 *
 * @returns {Object} Latest sync event or null if no syncs
 */
router.get('/claude-usage/syncs/latest', requireAuth, async (req, res, next) => {
  try {
    // =============================================================================
    // STEP 1: Fetch Latest Sync
    // =============================================================================
    const latestSync = await claudeUsageService.getLatestSyncInfo(req.user._id);
    ...
    attachEntityId(req, 'userId', req.user._id);
    if (latestSync) {
      attachEntityId(req, 'syncId', latestSync._id);
    }
    // Set event name for Wide Events logging
    req.eventName = 'claude_usage.syncs.latest';

    // =============================================================================
    // STEP 3: Return Latest Sync
    // =============================================================================
    res.json({
      success: true,
      data: latestSync
...
/**
 * GET /analytics/claude-usage/syncs/compare
 * Compare two specific sync events
 *
 * This endpoint compares two sync events to show:
 * - How much usage changed between syncs
 * - Cost delta
 * - Token delta
 ...
 * ---------
 * - Compare this week vs last week
 * - See usage growth over time
 * - Identify cost spikes
 *
 * @query {string} sync1 - First sync ID
 * @query {string} sync2 - Second sync ID
 * @returns {Object} Comparison with deltas
 */
router.get('/claude-usage/syncs/compare', requireAuth, async (req, res, next) => {
  try {
    // Extract sync IDs from query params
    const { sync1, sync2 } = req.query;
    ...
    if (!sync1 || !sync2) {
      return res.status(400).json({
        success: false,
        error: {
          message: 'Both sync1 and sync2 parameters are required',
          code: 'VALIDATION_ERROR'
        }
      });
    }
    ...
    // =============================================================================
    // STEP 2: Compare Syncs
    // =============================================================================
    const comparison = await claudeUsageService.compareSyncEvents(
      req.user._id,
      sync1,
      sync2
    );
    ...
    // =============================================================================
    // STEP 3: Log the Comparison Event
    // =============================================================================
    // Attach user ID and sync IDs for audit trail
    attachEntityId(req, 'userId', req.user._id);
    attachEntityId(req, 'sync1Id', sync1);
    attachEntityId(req, 'sync2Id', sync2);
    // Set event name for Wide Events logging
    req.eventName = 'claude_usage.syncs.compare';
